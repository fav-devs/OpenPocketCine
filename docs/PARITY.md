...
| Desktop watcher | Relay wire contract and the grade: framing, kind validation, payload ceiling, join and passcode rules, the three-rung retry ladder, the delivery-delay guard, fitted focus, and `.cube` parsing all come from the core through the `@_cdecl` facade. The feed pipeline is the Android shell's own `fullscreen.vert` / `feed.frag` / `blit.frag`, compiled from where they live, in the phones' order: convert at the source raster, cube at that raster, then stretch. | **Exception: watcher only.** No BLE, no SoftAP join, no camera datalink; the phone hosting the feed owns all three. Decoding is libavcodec rather than VideoToolbox or MediaCodec, and the YCbCr pass is written out because software decode yields planes, not an AHardwareBuffer. LUT, zebra, and peaking are driven; false colour and the scopes are not, and neither is any operator chrome or control UI — the window is the picture plus a title bar, with assists on keys. Peaking sensitivity is transcribed from the Android shell rather than read from the core, which is a drift risk until it moves there. Present prefers mailbox over FIFO. Discovery is pure-Rust mDNS rather than `NWBrowser`, on the same camera Wi-Fi with no peer-to-peer path. The camera commands and the UDP transport are exposed through the facade but no session carries them yet, so the desktop still cannot run a camera ([`desktop-camera-link.md`](desktop-camera-link.md)). | `just desktop-check`; the swapchain is checked through a headless surface, so no real window has been opened; **physical** verification against a live host still outstanding |
