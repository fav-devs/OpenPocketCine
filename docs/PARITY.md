...
| Desktop watcher | Relay wire contract and the grade: framing, kind validation, payload ceiling, join and passcode rules, the three-rung retry ladder, the delivery-delay guard, fitted focus, and `.cube` parsing all come from the core through the `@_cdecl` facade. The feed pipeline is the Android shell's own `fullscreen.vert` / `feed.frag` / `blit.frag`, compiled from where they live, in the phones' order: convert at the source raster, cube at that raster, then stretch. | **Exception: watcher only, no window yet.** No BLE, no SoftAP join, no camera datalink; the phone hosting the feed owns all three. Decoding is libavcodec rather than VideoToolbox or MediaCodec, and the YCbCr pass is written out because software decode yields planes, not an AHardwareBuffer. Output is a PNG still, not a live surface — no swapchain, chrome, or control UI. Scopes, peaking, false colour, and zebra exist in the shared shader but are not driven yet. Discovery is pure-Rust mDNS rather than `NWBrowser`, on the same camera Wi-Fi with no peer-to-peer path. | `just desktop-check`; **physical** verification against a live host still outstanding |
