...
| Desktop watcher | Relay wire contract only: framing, kind validation, payload ceiling, join and passcode rules, the three-rung retry ladder, the delivery-delay guard, and fitted focus all come from the core through the `@_cdecl` facade — the host never reimplements them. | **Exception: watcher only.** No BLE, no SoftAP join, no camera datalink; the phone hosting the feed owns all three. No decoder and no picture on screen yet (milestone 1 ingests access units and can dump them). No local assists, scopes, LUT, or control UI. Discovery is pure-Rust mDNS rather than `NWBrowser`, on the same camera Wi-Fi with no peer-to-peer path. | `just desktop-check`; **physical** verification against a live host still outstanding |
